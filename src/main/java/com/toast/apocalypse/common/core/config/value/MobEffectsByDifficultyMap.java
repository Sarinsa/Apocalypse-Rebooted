...
import com.toast.apocalypse.common.core.config.value.PairListValueCodec.Pair;
...
public class MobEffectsByDifficultyMap extends FuzzyMap<Double, List<Pair<FuzzyKey<MobEffect>, Integer>>> {
    
    public static final IFuzzyKeyParser<Double> PARSER = NumberKey.doubleParser( DoubleValueCodec.NON_NEGATIVE );
    public static final IRegWrapper<MobEffect> REGISTRY = IRegWrapper.of( ForgeRegistries.MOB_EFFECTS );
    public static final PairListValueCodec<FuzzyKey<MobEffect>, Integer> CODEC = PairListValueCodec.of(
            REGISTRY.getParser(), IntValueCodec.NON_NEGATIVE );
    
    /** Constructs an empty map. Use this if you want to {@link #load} a map from file/NBT. */
    public MobEffectsByDifficultyMap() { super( PARSER, CODEC ); }
    
    /**
     * Constructs a map containing the entries provided. You may use this for creating default values
     * during config definition, however the {@link MobEffectsByDifficultyMap.Builder} is much easier.
     */
    @SafeVarargs
    public MobEffectsByDifficultyMap( FuzzyEntry<Double, List<Pair<FuzzyKey<MobEffect>, Integer>>>... keys ) {
     ...
     * during config definition, however the {@link MobEffectsByDifficultyMap.Builder} is much easier.
     */
    public MobEffectsByDifficultyMap( Collection<FuzzyEntry<Double, List<Pair<FuzzyKey<MobEffect>, Integer>>>> keys ) {
        super( PARSER, CODEC, keys );
    }
    
    /** @return A fresh, empty collection of the same type as this one. */
    @Override
    public MobEffectsByDifficultyMap makeNew() { return new MobEffectsByDifficultyMap(); }
    
    
    // ---- Builder Implementation ---- //
    
    /** Builder to make constructing item stack maps smoother. */
    public static class Builder<B extends Builder<B>> extends AbstractBuilder<Double, List<Pair<FuzzyKey<MobEffect>, Integer>>, MobEffectsByDifficultyMap, B> {
        
        public Builder() { super( CODEC ); }
        
        /** @return A new fuzzy map reflecting the current state of this builder. */
        @Override
        public MobEffectsByDifficultyMap build() { return new MobEffectsByDifficultyMap( list ); }
        ...
        public class SubBuilder extends RegistryValueList.Builder<MobEffect, Integer, SubBuilder> {
            private final NumberKey<Double> root;
            
            SubBuilder( NumberKey<Double> key ) {
                super( REGISTRY, CODEC.rightCodec );
                root = key;
            }
            
            /** Finishes out the sub-builder entry. */
            public B buildSub() { return Builder.this.put( root, PairListValueCodec.fromEntries( super.build() ) ); }
            
            /** @return A new fuzzy list reflecting the current state of this builder. */
            @Override
            public RegistryValueList<MobEffect, Integer> build() { throw new UnsupportedOperationException( "Use #buildSub() to build!" ); }
        }
    }
}
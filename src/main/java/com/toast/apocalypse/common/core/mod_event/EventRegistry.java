...
public class EventRegistry {
    ...
    public static final EventType<?> NONE = register(-1, EmptyEvent::new, null, 0,
            (serverWorld, currentEvent, player, difficultyManager) -> false,
            (serverWorld, currentEvent, player, difficultyManager) -> true);

    public static final EventType<?> FULL_MOON = register(0, FullMoonEvent::new, References.FULL_MOON, 100,
            (serverWorld, currentEvent, player, difficultyManager) -> difficultyManager.isFullMoonNight(),
            (serverWorld, currentEvent, player, difficultyManager) -> difficultyManager.isFullMoonNight());

    public static final EventType<?> THUNDERSTORM = register(1, ThunderstormEvent::new, null, 2,
            (serverWorld, currentEvent, player, difficultyManager) -> serverWorld.isThundering(),
            (serverWorld, currentEvent, player, difficultyManager) -> serverWorld.isThundering());

    public static final EventType<?> ACID_RAIN = register(2, AcidRainEvent::new, References.ACID_RAIN, 1,
    ...
    private static <T extends AbstractEvent> EventType<T> register(int id, EventType.IEventFactory<T> factory, String startMessage, int priority, IEventPredicate startupPredicate, IEventPredicate persistPredicate) {
        ...
        EventType<T> eventType = new EventType<>(id, factory, startMessage, priority, startupPredicate, persistPredicate);
